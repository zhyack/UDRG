...
with g.as_default():
    ...
    with tf.Session() as sess:
        ...
        log_sae_losses = CONFIG['SAE_LOG']
        log_sae_losses_same = CONFIG['SAE_SAME_LOG']
        log_bleu = CONFIG['BLEU_LOG']
        print(log_sae_losses)
        print(log_sae_losses_same)
        print(log_losses)
        print(log_bleu)
        for n_iter in range(CONFIG['GLOBAL_STEP']/CONFIG['MAX_STEPS_PER_ITER'], CONFIG['ITERS']):
            ...
            for b in range(len(CONFIG['BUCKETS'])):
                ...
                for k in range((n_b+CONFIG['BATCH_SIZE']-1)/CONFIG['BATCH_SIZE']):
                    ...
                    eval_batch_copy = copy.deepcopy(eval_batch)
                    ...
                    sae_batch_loss, batch_loss, predict_outputs = Model.eval_on_batch(sess, model_inputs, len_inputs, inputs_mask, model_outputs, len_outputs, outputs_mask, model_targets, len_targets, targets_mask)

                    m = CONFIG['BATCH_SIZE']/2
                    for i in range(m):
                        eval_batch_copy[i+m]=[eval_batch_copy[i][1], eval_batch_copy[i][1]]
                        eval_batch_copy[i][1]=eval_batch_copy[i][0]
                    eval_batch_copy = map(list, zip(*eval_batch_copy))
                    model_inputs, len_inputs, inputs_mask = dataSeqs2NpSeqs(eval_batch_copy[0], full_dict_src, CONFIG['BUCKETS'][b][0])
                    model_outputs, len_outputs, outputs_mask = dataSeqs2NpSeqs(eval_batch_copy[1], full_dict_dst, CONFIG['BUCKETS'][b][1])
                    model_targets, len_targets, targets_mask = dataSeqs2NpSeqs(eval_batch_copy[1], full_dict_dst, CONFIG['BUCKETS'][b][1], bias=1)
                    sae_batch_loss_same, _, _ = Model.eval_on_batch(sess, model_inputs, len_inputs, inputs_mask, model_outputs, len_outputs, outputs_mask, model_targets, len_targets, targets_mask)

                    eval_losses.append(batch_loss)
                    sae_losses.append(sae_batch_loss)
                    sae_same_losses.append(sae_batch_loss_same)
                    eval_batch = map(list, zip(*eval_batch))
            ...
            print('Evaluation completed:\nAverage Loss:%.6f\nSAE Loss:%.6f\nSAME Loss:%.6f\nBLEU:%.2f'%(sum(eval_losses)/len(eval_losses),sum(sae_losses)/len(sae_losses),sum(sae_same_losses)/len(sae_same_losses),eval_bleu))
            print(log_losses[max(0,len(log_losses)-100):])
            print(log_bleu[max(0,len(log_losses)-100):])
            print(log_sae_losses[max(0,len(log_losses)-100):])
            print(log_sae_losses_same[max(0,len(log_losses)-100):])
            log_losses.append(sum(eval_losses)/len(eval_losses))
            log_sae_losses.append(sum(sae_losses)/len(sae_losses))
            log_sae_losses_same.append(sum(sae_same_losses)/len(sae_same_losses))
            ...
            if args.save_folder != None:
                ...
                CONFIG['SAE_LOG']=log_sae_losses
                CONFIG['SAE_SAME_LOG']=log_sae_losses_same

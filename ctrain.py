...
train_buckets_raw_parallel_2 = arrangeBuckets(train_raw_parallel, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_parallel_2])
f_x.close()
f_y.close()
...
train_buckets_raw_unparallel_2 = arrangeBuckets(train_raw_unparallel, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_unparallel_2])
f_x.close()
...
train_buckets_raw_dae_2 = arrangeBuckets(train_raw_dae, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_dae_2])
f_x.close()
f_y.close()

f_x = codecs.open(CONFIG['TRAIN_Y_P'],'r','UTF-8')
x_train_raw_parallel = f_x.readlines()
# print(findmaxlen(x_train_raw))
f_y = codecs.open(CONFIG['TRAIN_X_P'],'r','UTF-8')
y_train_raw_parallel = f_y.readlines()
# print(findmaxlen(y_train_raw))
train_raw_parallel = [ [x_train_raw_parallel[i].strip(),y_train_raw_parallel[i].strip()] for i in range(len(x_train_raw_parallel))]
train_buckets_raw_parallel_1 = arrangeBuckets(train_raw_parallel, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_parallel_1])
f_x.close()
f_y.close()

f_x = codecs.open(CONFIG['TRAIN_Y_U'],'r','UTF-8')
x_train_raw_unparallel = f_x.readlines()
y_train_raw_unparallel = x_train_raw_unparallel
train_raw_unparallel = [ [x_train_raw_unparallel[i].strip(),y_train_raw_unparallel[i].strip()] for i in range(len(x_train_raw_unparallel))]
train_buckets_raw_unparallel_1 = arrangeBuckets(train_raw_unparallel, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_unparallel_1])
f_x.close()

f_x = codecs.open(CONFIG['TRAIN_X_DA'],'r','UTF-8')
x_train_raw_dae = f_x.readlines()
f_y = codecs.open(CONFIG['TRAIN_X_A'],'r','UTF-8')
y_train_raw_dae = f_y.readlines()
train_raw_dae = [ [x_train_raw_dae[i].strip(),y_train_raw_dae[i].strip()] for i in range(len(x_train_raw_dae))]
train_buckets_raw_dae_1 = arrangeBuckets(train_raw_dae, CONFIG['BUCKETS'])
print([len(b) for b in train_buckets_raw_dae_1])
f_x.close()
f_y.close()

train_buckets_raw = [train_buckets_raw_parallel_2, train_buckets_raw_parallel_1, train_buckets_raw_dae_1, train_buckets_raw_dae_2, train_buckets_raw_unparallel_2, train_buckets_raw_unparallel_1]
dict_train_src = [full_dict_src, full_dict_dst, full_dict_src, full_dict_dst, full_dict_src, full_dict_dst]
dict_train_dst = [full_dict_dst, full_dict_src, full_dict_src, full_dict_dst, full_dict_dst, full_dict_src]
    ...
    with tf.Session() as sess:
        ...
        for n_iter in range(CONFIG['GLOBAL_STEP']/CONFIG['MAX_STEPS_PER_ITER'], CONFIG['ITERS']):
            while True:
                ...
                batch_loss = Model.train_on_batch(sess, model_inputs, len_inputs, inputs_mask, model_outputs, len_outputs, outputs_mask, model_targets, len_targets, targets_mask, mode=b_mode)
